...
describe('DashboardPage', () => {
    ...
    test('renders dashboard page', async () => {
        ...
        // should render top artist section
        const artistSection = screen.getByRole('heading', { level: 2, name: /top artist/i });
        expect(artistSection).toBeInTheDocument();

        // should render top artist card title
        const artistCard = screen.getByRole('heading', { level: 3, name: topArtistData.items[0].name });
        expect(artistCard).toBeInTheDocument();

        // should render top track section
        const trackSection = screen.getByRole('heading', { level: 2, name: /top track/i });
        expect(trackSection).toBeInTheDocument();

        // should render top track card title
        const trackCard = screen.getByRole('heading', { level: 3, name: topTrackData.items[0].name });
    ...
    test('displays error messages on fetch failure', async () => {
        ...
        expect(error).toHaveTextContent(/failed to fetch top/i);
    });
